...
[InitializeOnLoad]
public class HierarchyLableDev
{
    //  显示图标以及彩色文字
    ...
    // 绘制图标
    private static void DrawIcon<T>(Rect rect,GameObject current) where T :UnityEngine.Object
    {
        // 获得Unity内置的图标
        
        if(typeof(T) == typeof(Button))
        {
            Rect rectBtn = new Rect(rect);
            rectBtn.x-=1;
            rectBtn.width += 40;
            GUI.Label(rectBtn, "btn");
        }
        else if(typeof(T) == typeof(UIBase))
        {
            Rect rectBtn = new Rect(rect);
            rectBtn.x -= 7;
            rectBtn.width += 40;
            GUI.Label(rectBtn, "root");
        }
        else
        {
    ...
    private static void DrawRectIcon<T>(Rect selectionRect, GameObject go, Color textColor, ref int order, ref GUIStyle style) where T : Component
    {
        ...
        {
            // 绘制新的Label覆盖原有名字
            if (EnableCustomHierarchyLabel&&typeof(T)!=typeof(Button))
            {
                // 字体样式
    ...
    static void HierarchWindowOnGui(int instanceId, Rect selectionRect)
    {
        ...
        try
        {
            ...
            // 文字颜色定义
            var colorUIBase = new Color(42 / 255f, 210 / 255f, 235 / 255f);
            ...
            var colorCamera = Color.green;
            var colorCanvas = Color.red;
            ...
            //CanvasRoot
            DrawRectIcon<Canvas>(selectionRect, go, colorCanvas, ref index, ref style);
            DrawRectIcon<Button>(selectionRect, go, Color.gray, ref index, ref style);
            // Renderer
            DrawRectIcon<UIBase>(selectionRect, go, colorUIBase, ref index, ref style);
